...
public struct EchoTableView<InspectorFooter: View>: View {
    @ObservedObject var viewModel: EchoTableViewModel
    let inspectorFooter: (EchoTableRow) -> InspectorFooter

    /// Initializes the `EchoTableViewModel` with a view model and optional inspector footer view.
    /// - Parameters:
    ///   - viewModel: The `EchoTableViewModel` managing the table state.
    ///   - inspectorFooter: An optional footer view for the inspector, default is an empty view.
    public init(
        viewModel: EchoTableViewModel,
            ...
            Table(
                ...
                TableColumnForEach(viewModel.columnKeys()) { columnKey in
                    TableColumn(columnKey.id) { row in
                        if let value: String = row.columnItems[columnKey.id] {
                            Text(value)
    ...
    private struct EchoTableFooter: View {
        ...
        public var body: some View {
            EchoListFooterView(
                itemCount: viewModel.rows.count,
                itemLabel: "item",
                actionButtons: [
                    .init(
                        iconName: viewModel.isPaused ? "play.fill" : "pause.fill",
                        helpText: "Pause. New items will be dropped until unpaused.",
                        action: { viewModel.isPaused.toggle() }
                    ),
                    .init(
                        iconName: "trash",
                        helpText: "Clear all",
                        isDestructive: true,
                        action: { viewModel.clearRows() }
                    )
                ]
            ) {
                if !viewModel.filterableColumns.isEmpty {
                    HStack {
